...
class CarService:
    ...
    def get_car_details1(self, car_id):
        db = sqlite3.connect("cars.db")
        c = db.cursor()
        query = f' select c.*, r.user_id, r.review from cars c left join reviews r on ' \
                f'c.cars_id = r.car_id where c.cars_id = {car_id}'
        c.execute(query)
        out = c.fetchall()
        first_row = out[0]
        car = Car(first_row[0], first_row[1], first_row[2], first_row[3], first_row[4])
        reviews = map(lambda row: Review(row[5], car_id, row[6]), out)
        return car, reviews
